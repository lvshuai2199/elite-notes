...
ROBOT_BUS_GENERATION_ID=${ARG_ROBOT_BUS_GENERATION_ID:-0}
...
help() {
  ...
  echo "options:"
  echo "  -m <model>   Robot model (CS63, CS66, ES66, LS62, LS65, etc). Defaults to CS66."
  echo "  -c           CONTROL_BOX_ID. Defaults to 1"
  echo "  -t           TOOL_TYPE_ID. Defaults to 1"
  echo "  -b           ROBOT_BUS_GENERATION_ID. Defaults to 0 (0=High Speed Bus, 1=EtherCAT Bus)"
  ...
  echo "  -r <0|1|2>   Robot series flag. 0 = CS_SERIES, 1 = ES_SERIES, 2 = LS_SERIES (auto overridden by -m)"
  echo "  -R <base>    Registry image base. Default: '$REGISTRY_IMAGE_BASE'"
  echo "  -h           Show this help"
}

parse_arguments() {
  while getopts ":hm:v:p:u:i:f:n:dt:c:r:b:R:" option; do
    case $option in
      h) help; exit ;;
      ...
      b) ROBOT_BUS_GENERATION_ID=${OPTARG} ;;
...
fill_information() {
  ...
  elif [[ "$ROBOT_MODEL" =~ ^LS[0-9A-Z-]*$ ]]; then
    SERIES_FLAG=2
    ROBOT_SERIES="LS_SERIES"
  else
    SERIES_FLAG=0
    ROBOT_SERIES="CS_SERIES"
  fi
...
resolve_docker_image() {
  ...
  # 先检查 elite-sim (带中划线)
  if docker image inspect "elite-sim:${version}" >/dev/null 2>&1; then
    echo "elite-sim:${version}"
    return
  fi
...
main() {
  ...
  echo -e "ROBOT_BUS_GENERATION_ID: \e[35m$ROBOT_BUS_GENERATION_ID\e[0m"
  docker_args=(
    run --rm
    ...
    -e "ROBOT_BUS_GENERATION_ID=${ROBOT_BUS_GENERATION_ID}"
    --name "$CONTAINER_NAME"
  )
  ...
  if [ "$DETACHED" = true ]; then
    ...
    docker "${docker_args[@]}" >/dev/null 2>&1 &
    DOCKER_PID=$!
    sleep 2
    if docker ps --format '{{.Names}}' | grep -qx "$CONTAINER_NAME"; then
      echo -e "\e[32m✅ Container started successfully\e[0m"
    else
      echo "❌ Failed to start container: $CONTAINER_NAME"
      exit 2
    fi
